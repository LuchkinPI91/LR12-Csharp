...
public class Main{
    ...
    public static void main(String[] args) throws Exception{
        ...
        Students students1= new Students(n,name,groupn,avg_balls1,avg_balls2,id1,specialtys);
        Spec specialty = new Spec(specialtys);
       ...
       // students1.outPut(n);//вывод списка студентов
        ...
        students1.FindByLastname(scanner.nextLine());
...
class Students extends Decanat implements FindBy{
   ...
   public Students(int n,String[] last_name, int[] group, float[] avg_ball1, float[] avg_ball2,int[] id1,String[] sp){// конструктор с параметрами
        val = n;
        for(int i = 0;i < n; i++){
    ...
    public void FindByStudentBook(int nomergrupi){
        ...
        for (int i =0;i<val;i++){
            ...
            else System.out.println("Номер группы не найден!");
        }
    }
    @Override
    public void FindByLastname(String last_name){
        ...
        for (int i =0;i<val;i++){
            if(this.last_name[i] == last_name){

                System.out.println("Фамилия:"+this.last_name[i] + " Оценка за 1 экзамен" + avg_ball1[i] + " Оценка за 1 экзамен" + avg_ball2[i]);
    ...
    String q = "Фамилия:" + last_name[qwer] + " Группа:" + group[qwer] + " Оценка за 1 экзамен:" + avg_ball1[qwer] + " Оценка за 2 экзамен:" + avg_ball2[qwer] + " Норме зачетной книжки:" + student_book.get_id(qwer) + " Специальность" + spec1.getSpec(qwer);
    qwer ++;
    return q;
